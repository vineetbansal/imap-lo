...
def print_lines(fgt, fcn, date, begin1, end1, sum_bg_cnts, sum_og_cnts, sum_bg_expo,  bg_rate_nom, pivot,pivotp, spin_period_ave, yyyymmdd, repoint):
    ...
    print (
        f"{date},{yyyymmdd}, {repoint}, {int(begin2)},{int(end2)},30,59,Lo,{sum_bg_cnts},{sum_og_cnts},{sum_bg_expo},{pivot},{pivotp}, {spin_period_ave}", file=fcn
        )
...
def argParsing():
    ...
    parser.add_argument('-toff1', '--tcutOff1',
                        help='the cut rate in s-1 (e.g., 0.0028) for PPM angles other than 90 deg (e.g., 75 deg)',
                        dest='tcutOff1',
                        type=float,
                        default=0.0035)
    
    parser.add_argument('-toff2', '--tcutOff2',
                        help='the cut rate in s-1 (e.g., 0.0028) for PPM angles other than 90 deg (e.g., 105 deg)',
                        dest='tcutOff2',
                        type=float,
                        default=0.0035)
    ...
    parser.add_argument('-tramoff1', '--tcutramOff1',
                         help='the cut rate for PPM angles other than 90 deg ram directio (75 deg) Esteps 6 and 7 (e.g., 0.0035 * 2.0) ',
                         dest='tcutramOff1',
                         type=float,
                         default=0.007)
    
    parser.add_argument('-tramoff2', '--tcutramOff2',
                         help='the cut rate for PPM angles other than 90 deg ram directio (105 deg) Esteps 6 and 7 (e.g., 0.0035 * 2.0) ',
                         dest='tcutramOff2',
                         type=float,
                         default=0.007)
...
#    print(t0_hk)
# print(times[-1])
    start_time_hk = t0_hk + timedelta(hours=0.5)
    end_time_hk = t0_hk + timedelta(hours=22.5)
# print(start_time_hk)
# print(end_time_hk)
    ...
    spin_period_ave = np.nanmedian(spin_period[mask_hk])
...
elif 73.0 < pivot < 77.0:
    bg_rate_nom = args.tcutOff1
    bg_rate_ram_nom = args.tcutramOff1
elif 103.0 < pivot < 107.0:
    bg_rate_nom = args.tcutOff2
    bg_rate_ram_nom = args.tcutramOff2
else: 
    bg_rate_nom = args.tcut90
    bg_rate_ram_nom = args.tcutram90
...
#                last_end = end
                ...
                print_lines(fgt, fcn, date1, begin, end, sum_bg1_cnts, sum_og1_cnts, sum_bg1_expo,  bg_rate_nom, pivot, pivotp, spin_period_ave, date_yyyymmdd, repoint_int)
