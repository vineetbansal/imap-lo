...
for esa in range(1, 8):
    ...
    n = len(TOF0)
    if n < minEvents:
        print("1S08 1DTOF Failed total number of events = ", n, "esa = ", esa)
        continue
    ...
    ncheck = len(x0)
    if (ncheck < minEvents): 
        print("1S08 1DTOF Failed number of events after valid selection = ", ncheck, "esa = ", esa)
        continue

    x = x2
    if (args.tof ==0):
        xlabel = 'TOF0 (ns) '+'ESA='+str(esa)+', Valid:' +str(valid[0])+str(valid[1])+str(valid[2])+str(valid[3])+str(valid[4])
        x = x0
        if (delayCorr ==1):
            x = x + 0.5*x3
            xlabel = 'TOF0s (ns) '+'ESA='+str(esa)+', Valid:' +str(valid[0])+str(valid[1])+str(valid[2])+str(valid[3])+str(valid[4])
    if (args.tof ==1):
        xlabel = 'TOF1 (ns) '+'ESA='+str(esa)+', Valid:' +str(valid[0])+str(valid[1])+str(valid[2])+str(valid[3])+str(valid[4])
        x = x1
        if (delayCorr ==1):
            x = x - 0.5*x3
            xlabel = 'TOF1s (ns) '+'ESA='+str(esa)+', Valid:' +str(valid[0])+str(valid[1])+str(valid[2])+str(valid[3])+str(valid[4])
    if (args.tof ==2):
    ...
    plt.hist(x, bins=bins, facecolor = 'b', edgecolor='k', linewidth=0.5, range=(xMin,xMax_local))
    ...
    #    plt.savefig('TOF'+str(args.tof)+'.png', dpi=args.dpi)
        plt.savefig(args.outFile+'TOF'+str(args.tof)+'ESA'+str(esa)+'.png', dpi=dpi, facecolor='w', edgecolor='w', orientation='portrait', format=None, transparent=False, bbox_inches=None, pad_inches=0.1)
    ...
    #    plt.savefig("test2")
        plt.savefig(args.outFile+'TOF'+str(args.tof)+'s'+'ESA'+str(esa)+'.png', dpi=dpi, facecolor='w', edgecolor='w', orientation='portrait', format=None, transparent=False, bbox_inches=None, pad_inches=0.1)
