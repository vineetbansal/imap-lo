...
for esa in range(1,8):
    ...
    n = len(TOF1)
    if (n < minEvents):
        print("delay Failed total number of events = ", n, "esa = ", esa)
        continue
    ...
    ncheck = len(x0)
    if (ncheck < minEvents): 
        print("Delay Failed number of events after valid selection = ", ncheck, "esa = ", esa)
        continue
    ...
    if n_valid_tof3 < minEvents:
        print("Delay Failed after valid selection with number of valid TOF3 = ", n_valid_tof3)
        continue
