...
def load_bg_maps(work_dir_maps, work_dir_cosalpha):
    """Loads flux, func (uncertainty), and fvar (variance) from CSV maps across 7 energy levels."""
    print(f"Loading files from {work_dir_maps}...")
    flux = np.zeros((30, 60, 7))
    func = np.zeros((30, 60, 7))
    # this includes only the systematic uncertainty
    fvar = np.zeros((30, 60, 7))
    ...
    for k in range(7):
        ...
        # Read maps, skip column names if present, but typically the first row/column contain coordinates/indices.
        # Based on inspection, row 1 is x-index, and data is 31 rows by 60 columns.
        f_df = pd.read_csv(f'{work_dir_maps}/map_bflux_esa{esa}.csv', skiprows=1, header=None)
        u_df = pd.read_csv(f'{work_dir_maps}/map_bfunc_esa{esa}.csv', skiprows=1, header=None)
        v_df = pd.read_csv(f'{work_dir_maps}/map_bfvar_esa{esa}.csv', skiprows=1, header=None)
        w_df = pd.read_csv(f'{work_dir_cosalpha}/map_cosalpha_esa{esa}.csv', skiprows=1, header=None)    
...
def load_survival_probability_maps(work_dir_survival):
    """Loads survival probability maps into a 30 x 60 x 7 cube."""
    print(f"Loading survival probability files from {work_dir_survival}...")

    survival_prob = np.zeros((30, 60, 7))

    for k in range(7):
        esa = k + 1
        s_df = pd.read_csv(
            f'{work_dir_survival}/map_survival_probability_esa{esa}.csv',
            skiprows=1,
            header=None
        )
        survival_prob[:, :, k] = s_df.values[:, :]

    print(f"Completed loading survival probability shape: {survival_prob[:, :, 0].shape}")
...
def apply_compton_getting():
    ...
    for pp in [75,90,105]:
        ...
        work_dir_survival = f'../4S5_glows_SP_reader/outdir_glows/pivot_{pp}/maps/'
        survival_prob = load_survival_probability_maps(work_dir_survival)
        ...
        for back in range(2):
            if (back==0):
                ...
                work_dir=f'../3S5_l1b_ram_maps/outdir/pivot_{pp}/maps/'
                work_dir_cosalpha=f'../3S5_l1b_ram_maps/outdir/pivot_{pp}/maps/'
                out_dir=f'./outdir/pivot_{pp}/maps/'
                flux, func, fvar, cosalpha = load_bg_maps(work_dir, work_dir_cosalpha)
            ...
            gamma_surv = np.zeros_like(gamma)
            ...
            for k in range(7):
                ...
                # ----- survival probability slope -----

                if k <= 1:

                    k1_default, k2_default = 0, 1
                    k1_low,     k2_low     = 0, 1

                    use_lower_slope = np.zeros_like(Es_k[:, :, k], dtype=bool)

                else:

                    k1_default, k2_default = k - 1, k
                    k1_low,     k2_low     = k - 2, k - 1

                    use_lower_slope = Es_k[:, :, k] < E_k[k - 1]

                if np.any(use_lower_slope):
                        print(
                            f"WARNING pivot {pp} ESA {k+1}: "
                            f"{np.sum(use_lower_slope)} pixels have Es_k below E_k[k-1]. "
                            f"Using survival slope ESA {k-2} to ESA {k-1} instead of ESA {k-1} to ESA {k}."
                        )

                with np.errstate(divide='ignore', invalid='ignore'):
                    gamma_default = (
                        np.log(
                            survival_prob[:, :, k2_default]
                            /
                            survival_prob[:, :, k1_default]
                        )
                        /
                        np.log(E_k[k2_default] / E_k[k1_default])
                    )

                    gamma_low = (
                        np.log(
                            survival_prob[:, :, k2_low]
                            /
                            survival_prob[:, :, k1_low]
                        )
                        /
                        np.log(E_k[k2_low] / E_k[k1_low])
                    )

                gamma_default[~np.isfinite(gamma_default)] = 0.0
                gamma_low[~np.isfinite(gamma_low)] = 0.0

                gamma_surv[:, :, k] = np.where(
                    use_lower_slope,
                    gamma_low,
                    gamma_default
                )

                gamma_surv[:, :, k][~np.isfinite(gamma_surv[:, :, k])] = 0.0

                with np.errstate(divide='ignore', invalid='ignore'):

                    survival_prob_est_default = (
                        survival_prob[:, :, k2_default]
                        *
                        (Es_k[:, :, k] / E_k[k2_default]) ** gamma_default
                    )

                    survival_prob_est_low = (
                        survival_prob[:, :, k2_low]
                        *
                        (Es_k[:, :, k] / E_k[k2_low]) ** gamma_low
                    )

                survival_prob_est = np.where(
                    use_lower_slope,
                    survival_prob_est_low,
                    survival_prob_est_default
                )
                ...
                survival_prob_est[
                    (~np.isfinite(survival_prob_est)) |
                    (survival_prob_est <= 0.0)
                ] = np.nan

                J_cg[:, :, k] /= survival_prob_est
        ...
        # Save Flux
                ...
                df_j.to_csv(f'{out_dir}/{name}_cgflux_esa{esa}.csv', index=False)
                    # Save Func
                df_u = pd.DataFrame(np.nan_to_num(func_cg[:, :, k], nan=0.0))
                df_u.columns = cols_idx
                df_u.to_csv(f'{out_dir}/{name}_cgfunc_esa{esa}.csv', index=False)
        # Save Fvar
                df_v = pd.DataFrame(np.nan_to_num(fvar_cg[:, :, k], nan=0.0))
                df_v.columns = cols_idx
                df_v.to_csv(f'{out_dir}/{name}_cgfvar_esa{esa}.csv', index=False)
