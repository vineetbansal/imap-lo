...
def filter_and_write_cdf(cdf_file,  hk_file, out_file):
    """
    ...
    # Expand packet-level shcoarse to event-level shcoarse
...
#    )
    ...
    for esa in range (1,8):
        with open(out_file+'_ESA'+str(esa)+'.csv', 'w') as fle:
            print("shcoarse,tof3,tof2,tof1,tof0,absent,mode,nep_spinphase,esa_step", file=fle)
...
#            tof0_check = (tof0[:, None] >= H_PEAK_L[0]) & (tof0[:, None] <= H_PEAK_H[0])
#            tof1_check = (tof1[:, None] >= H_PEAK_L[1]) & (tof1[:, None] <= H_PEAK_H[1])
#            tof2_check = (tof2[:, None] >= H_PEAK_L[2]) & (tof2[:, None] <= H_PEAK_H[2])
#            tof3_check = (tof3[:, None] >= 0.0) & (tof3[:, None] <= 20.0)
    ...
    #        event_pass =  esa_check & absent_check & mode_check & tof2_check & tof3_check  
    ...
    #    print(mask[0:20])
        ...
        # -------------------------------
            ...
            print(f"1S17 H: ESA = {esa}, Filtered {mask.sum()} / {N} rows written to {out_file} + ESA")
