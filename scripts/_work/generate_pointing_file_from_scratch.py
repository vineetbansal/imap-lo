...
def spin_axis_for_day(date: datetime, ck: Path, spk: Path) -> tuple[float, float] | None:
    ...
    for hour in range(24):
        utc = date.replace(hour=hour, minute=30, tzinfo=timezone.utc).strftime(
            "%Y-%m-%dT%H:%M:%S"
        )
        et = spice.str2et(utc)
        try:
            rot = spice.pxform("IMAP_SPACECRAFT", "J2000", et)
            z_j2000 = np.array(rot) @ np.array([0.0, 0.0, 1.0])
            samples_xyz.append(z_j2000)
        except Exception:
            pass
    ...
    mean_vec = np.mean(samples_xyz, axis=0)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--out",
        type=Path,
        default=SCRIPTS_DIR / "pointing_file0.csv",
    )
    return p.parse_args()
