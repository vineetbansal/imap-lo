...
for pp in [75, 90, 105]:
    ...
    work_dir_base = f"../3S5_l1b_ram_maps/outdir/pivot_{pp}/maps"

    # CAVA-like output directory colocated with corrected_maps
    base_cava = "./outdir/cava_cg"
    ...
    for esa in range(1, 8):
        ...
        expo_file = os.path.join(work_dir_base, f"map_expo_esa{esa}.csv")
        if not os.path.exists(expo_file):
            print("fubar .. ")
            continue

        # CG corrected flux (flux + expo + variance)
        cg_flux = os.path.join(work_dir, f"map_cgflux_esa{esa}.csv")
        cg_flux_var = os.path.join(work_dir, f"map_cgfvar_esa{esa}.csv")

        if os.path.exists(cg_flux) and os.path.exists(cg_flux_var):
            for csv_src, stem, lbl in [
                (cg_flux, "flux", label_map["flux"]),
                (expo_file, "exposure", label_map["expo"]),
                (cg_flux_var, "variance", label_map["fvar"]),
            ]:
        ...
        # CG sys uncertainty (fser + expo + variance)
        cg_fser = os.path.join(work_dir, f"map_cgfunc_esa{esa}.csv")
        cg_fser_var = os.path.join(work_dir, f"map_cgfvar_esa{esa}.csv")

        if os.path.exists(cg_fser) and os.path.exists(cg_fser_var):
            for csv_src, stem, lbl in [
                (cg_fser, "flux", label_map["fser"]),
                (expo_file, "exposure", label_map["expo"]),
                (cg_fser_var, "variance", label_map["fvar"]),
            ]:
                if os.path.exists(csv_src):
                    out = os.path.join(
                        yaml_dirs["cg_fser"],
        ...
        # CG lower sys uncertainty (fsel + expo + variance)
        cg_fsel = os.path.join(work_dir, f"map_cgfunl_esa{esa}.csv")
        cg_fsel_var = os.path.join(work_dir, f"map_cgfvar_esa{esa}.csv")

        if os.path.exists(cg_fsel) and os.path.exists(cg_fsel_var):
            for csv_src, stem, lbl in [
                (cg_fsel, "flux", label_map["fsel"]),
                (expo_file, "exposure", label_map["expo"]),
                (cg_fsel_var, "variance", label_map["fvar"]),
            ]:
                if os.path.exists(csv_src):
                    out = os.path.join(
                        yaml_dirs["cg_fsel"],
        ...
        # CG upper sys uncertainty (fseu + expo + variance)
        cg_fseu = os.path.join(work_dir, f"map_cgfunu_esa{esa}.csv")
        cg_fseu_var = os.path.join(work_dir, f"map_cgfvar_esa{esa}.csv")

        if os.path.exists(cg_fseu) and os.path.exists(cg_fseu_var):
            for csv_src, stem, lbl in [
                (cg_fseu, "flux", label_map["fseu"]),
                (expo_file, "exposure", label_map["expo"]),
                (cg_fseu_var, "variance", label_map["fvar"]),
            ]:
                if os.path.exists(csv_src):
                    out = os.path.join(
                        yaml_dirs["cg_fseu"],
