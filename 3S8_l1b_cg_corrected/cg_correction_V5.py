...
def load_maps(work_dir_sputterboot, work_dir_cosalpha):
    ...
    funl = np.zeros((30, 60, 7))
    funu = np.zeros((30, 60, 7))
    ...
    for k in range(7):
        ...
        uldf = pd.read_csv(f'{work_dir_sputterboot}/map_flux_{esa}_Hy_boot_unl.csv', skiprows=1, header=None)
        uudf = pd.read_csv(f'{work_dir_sputterboot}/map_flux_{esa}_Hy_boot_unu.csv', skiprows=1, header=None)
    ...
    return flux, func, funl, funu, fvar, cosalpha
    ...
    for pp in [75,90,105]:
        ...
        for back in range(2):
            if (back==0):
                ...
                flux, func, funl, funu, fvar, cosalpha = load_maps(work_dir, work_dir_cosalpha)
            else: 
            ...
            funu_cg = np.zeros_like(funu)
            funl_cg = np.zeros_like(funl)
        ...
        # Here we scale absolute uncertainty proportionally.
                ...
                funl_cg[:, :, k] = funl[:, :, k] * ratio
                funu_cg[:, :, k] = funu[:, :, k] * ratio
        ...
        # Save Flux
                ...
                dful = pd.DataFrame(np.nan_to_num(funl_cg[:, :, k], nan=0.0))
                dful.columns = cols_idx
                dful.to_csv(f'{out_dir}/{name}_cgfunl_esa{esa}.csv', index=False)

                dfuu = pd.DataFrame(np.nan_to_num(funu_cg[:, :, k], nan=0.0))
                dfuu.columns = cols_idx
                dfuu.to_csv(f'{out_dir}/{name}_cgfunu_esa{esa}.csv', index=False)
