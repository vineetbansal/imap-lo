...
def load_maps(work_dir_sputterboot):
    ...
    for k in range(7):
        ...
        f_df = pd.read_csv(f'{work_dir_sputterboot}/map_flux_esa{esa}.csv', skiprows=1, header=None)
        u_df = pd.read_csv(f'{MAPS_DIR}/map_fser_esa{esa}.csv', skiprows=1, header=None)
        v_df = pd.read_csv(f'{MAPS_DIR}/map_fvar_esa{esa}.csv', skiprows=1, header=None)
        w_df = pd.read_csv(f'{COSALPHA_DIR}/map_cosalpha_esa{esa}.csv', skiprows=1, header=None)    
...
def apply_compton_getting():
    ...
    for pp in [75,90,105]:
        ...
        work_dir=f'../3S4_l1b_SputterBootstrap/outdir/pivot_{pp}/corrected/'
        out_dir=f'./outdir/pivot_{pp}/maps/'

        os.makedirs(out_dir,exist_ok=True)

        flux, func, fvar, cosalpha = load_maps(work_dir)

    cos_alpha, Es_k = compute_kinematics(cosalpha)
    
    print("Computing source fluxes and power-laws...")
    J_src, gamma = predictor_corrector(flux)
    
    print("Applying Compton-Getting scale factoring...")
