...
# Read CSV, everything after 13th column is ignored
    ...
    # -------------------------------
            N = len(met)
#            M = len(time_start) .. M died .. chatGBT killed it :)
...
# select golden triples only
            ...
            tof3_check = (tof3[:, None] >= 0.0) & (tof3[:, None] <= 20.0)  # reserved for future use

            if args.species == 1:  # O
                tof0_check = (tof0s[:, None] >= CO_PEAK_L[0]) & (tof0s[:, None] <= CO_PEAK_H[0])
                tof1_check = (tof1s[:, None] >= CO_PEAK_L[1]) & (tof1s[:, None] <= CO_PEAK_H[1])
                tof2_check = (tof2[:, None]  >= CO_PEAK_L[2]) & (tof2[:, None]  <= CO_PEAK_H[2])
            elif args.species == 0:  # H
                tof0_check = (tof0s[:, None] >= H_PEAK_L[0]) & (tof0s[:, None] <= H_PEAK_H[0])
                tof1_check = (tof1s[:, None] >= H_PEAK_L[1]) & (tof1s[:, None] <= H_PEAK_H[1])
