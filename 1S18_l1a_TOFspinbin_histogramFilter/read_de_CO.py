...
#    df['tof1'] = np.where(df['mode']==1, (df['tof1d']*ADC_TOF1[1]+ADC_TOF1[0]),(df['tof1']*ADC_TOF1[1]+ADC_TOF1[0]))
    ...
    print("spin ave = ", spin_period_ave)
...
#    )
    ...
    for esa in range (1,8):
        with open(out_file+'_ESA'+str(esa)+'.csv', 'w') as fle:
            print("shcoarse,tof3,tof2,tof1,tof0,absent,mode,nep_spinbin,esa_step", file=fle)
...
#                        filtered['de_time'],
            ...
            print(f"1S17 CO: ESA = {esa}, Filtered {mask.sum()} / {N} rows written to {out_file} + ESA")
