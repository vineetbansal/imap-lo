...
def filter_and_write_cdf(args):
    ...
    print("1S18 l1a sanity check for alignmen: ", len(shcoarse_expanded), len(tof0), len(tof1), len(de_time))
...
#    df['tof1'] = np.where(df['mode']==1, (df['tof1d']*ADC_TOF1[1]+ADC_TOF1[0]),(df['tof1']*ADC_TOF1[1]+ADC_TOF1[0]))
    ...
    print("1S18 spin period ave  = ", spin_period_ave)
...
# shape (N_events, N_goodtimes)
 #  print("met:", met.shape)
            ...
            if args.species == 1:  # O
                tof0_check = (tof0 >= CO_PEAK_L[0]) & (tof0 <= CO_PEAK_H[0]) # baggage not checked
                tof1_check = (tof1 >= CO_PEAK_L[1]) & (tof1 <= CO_PEAK_H[1]) # baggage not checked
                tof2_check = (tof2 >= CO_PEAK_L[2]) & (tof2 <= CO_PEAK_H[2])
                species='O'
            elif args.species == 0:  # H
                tof0_check = (tof0 >= H_PEAK_L[0]) & (tof0 <= H_PEAK_H[0]) # baggage not checked
                tof1_check = (tof1 >= H_PEAK_L[1]) & (tof1 <= H_PEAK_H[1]) # baggage not checked
...
#                        filtered['de_time'],
            ...
            print(f"1S18 species, {species}, ESA = {esa}, Filtered {mask.sum()} / {N} rows written to {out_file}_ESA{esa}.csv")
