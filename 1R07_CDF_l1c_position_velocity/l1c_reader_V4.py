...
def print_coord(fle, date, spin_axis, sc_lon, sc_lat, sc_lon2, sc_lat2, sc_lon_ephem, sc_lat_ephem, spin_lon, spin_lat, spin_lon_eq, spin_lat_eq, sc_position, sc_velocity, spin_axis_angle):
    ...
    print( 
        f"{date}, {spin_axis_angle}, {sc_lon}, {sc_lat}, {sc_lon2}, {sc_lat2}, {sc_lon_ephem}, {sc_lat_ephem}, {spin_lon}, {spin_lat}, {spin_lon_eq}, {spin_lat_eq}, {spin_axis}, {sc_position}, {sc_velocity}, {spin_axis_angle}", file=fle
        )
...
def convert_spin_to_J2000_eq(epoch, lon_spin, lat_spin):
    # converts from ecliptic to equatorial coords
...
def earth_heliocentric_coords(epoch):
    ...
    The output frame corresponds to:
        ...
        +Z:          north pole of the J2000 mean ecliptic
        Longitude:   atan2(y, x), increasing toward +Y
    ...
    # Convert Unix time to a UTC datetime.
    dt = datetime.fromtimestamp(epoch_time, tz=timezone.utc)
    ...
    L_deg = 100.46457166 + 35999.37244981 * T
    varpi_deg = 102.93768193 + 0.32327364 * T
    Omega_deg = 0.0

    # Argument of perihelion and mean anomaly.
    omega_deg = varpi_deg - Omega_deg
    M_deg = L_deg - varpi_deg

    # Wrap mean anomaly to [-180, 180).
    ...
    x = (
        (cos_omega * cos_Omega
         - sin_omega * sin_Omega * cos_inc) * x_orbit
        +
        (-sin_omega * cos_Omega
         - cos_omega * sin_Omega * cos_inc) * y_orbit
    )

    y = (
        (cos_omega * sin_Omega
         + sin_omega * cos_Omega * cos_inc) * x_orbit
        +
        (-sin_omega * sin_Omega
         + cos_omega * cos_Omega * cos_inc) * y_orbit
    )

    z = (
        sin_omega * sin_inc * x_orbit
        + cos_omega * sin_inc * y_orbit
    )
...
# this is the old one that is not J2000 and not really Mean-of-Date either

def earth_heliocentric_coords_old(epoch):
    """
...
def earth_ecliptic_longitude(epoch):
    """
    Approximate Earth heliocentric ecliptic longitude in the fixed
    J2000 ecliptic frame.

    This is a drop-in replacement for the previous longitude-only
    routine.
    ...
    Returns
    -------
    earth_long_j2000 : float
        Earth heliocentric ecliptic longitude in ECLIPJ2000 coordinates,
        in degrees over [0, 360).
    """

    lon_deg, lat_deg = earth_heliocentric_coords(epoch)
...
def compute_spin_axis_from_cdf_gen(
    ...
    # Load lat / lon
    # --------------------
    # remember this is the pointing set
    # there is only a 0 list
    lat = cdf[lat_var][0, :nspin, mid_index]
    lon = cdf[lon_var][0, :nspin, mid_index]
...
#        print(lon_sc_sun, lon_sc)
    ...
    lon_spin = (lon_spin - ((year - 2000.0) * 0.01397)) % 360.0
    spin_axis_1 = sph_to_cart(lat_spin, lon_spin)

#   this moves us from the Mean-of-Date to Ecliptic J2000
...
#    lon_sc = (np.rad2deg(np.arctan2(sy, sx)) + (0.5*365.25/365.0)) % 360.0
    lon_sc = lon_sc - ((year - 2000.0) * 0.01397)
    #   this moves us from the Mean-of-Date to Ecliptic J2000
    #   the corection factor was applied in Jun 2026
    pos_1 = sph_to_cart(lat_sc, lon_sc)
    print("lon = ", lon_sc)
    print("lat = ", lat_sc)
    print("pos_1 = ", pos_1)

    to_sun_1 = -1.0*pos_1
    # the added 0.5 is becuase we want to move to the center of the day

    # --------------------
    # Diagnostics
    # --------------------
    theta = angle_between(to_sun_1, spin_axis_1)
    theta_deg = np.degrees(theta)

    diagnostics = {
        "spin_axis": spin_axis_1,
        "mean_dot_orthogonality": np.mean(np.abs(V @ spin_axis_1)),
        "std_dot_orthogonality": np.std(V @ spin_axis_1),
        "solar_direction": to_sun_1,
        "sc_position": -1.0*to_sun_1, 
        "sc_ephemeris": pos_1,
        "sc_velocity":scv,
        "spin_angle": theta_deg
    }
    return spin_axis_1, lat_spin, lon_spin, lat_sc, lon_sc, diagnostics
...
try:
    ...
    print(f"1S07 IMAP NAIF ID: {inst_id}")
except:
    print("1S07 IMAP name not found in the loaded kernels.")
...
if (mag > 0.0):
    ...
    lat_ephem = np.rad2deg(np.arcsin(sz))
    lon_ephem = np.rad2deg(np.arctan2(sy, sx)) % 360.0

    if not (np.isfinite(lat_ephem) and np.isfinite(lon_ephem)):
        lat_ephem = -999.99
        lon_ephem = -999.99
else: 
    lat_ephem = -999.99
