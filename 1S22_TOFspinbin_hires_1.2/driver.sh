...
if [ ! -d "$input" ]; then
  echo "Error 1S22: Directory '$directory' not found."
  exit 1
fi
...
find "$input" -maxdepth 1 -type f -name "*.cdf" -mtime "$delaytime" -print0 | while IFS= read -r -d '' file; do
  ...
  if [ -f "$file" ]; then
      echo "1S22 Processing file: $file"
  fi
  ...
  echo "1S22 Output files: $ofileH $ofileCO"
...
#  ./runIMAP-emv3-instrument-report.sh "$ofile" "$datadir"
    ...
    if (( ${#files[@]} > 0 )); then
        ...
        nlines=$(wc -l < "${ofile}")
        if (( ${nlines} > 3)); then
            ./runIMAP-emv3-instrument-report.sh "$ofile" "$datadir" "300" "100" "100" "100" "20" 
        fi
