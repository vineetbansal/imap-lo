...
#   the corection factor was applied in Jun 2026
    if epoch0 < cutoff_acsfixed:
        print("before cutoff")
        decimal_year = (
            epoch0.year
            + (epoch0.timetuple().tm_yday - 1) / 365.25
        )
        lon_spin = (lon_spin - ((decimal_year - 2000.0) * 0.01397)) % 360.0
    else:   
        print("after cutoff")
...
#    lon_sc = (np.rad2deg(np.arctan2(sy, sx)) + 0.5) % 360.0
    if epoch0 < cutoff_acsfixed:
        ...
        lon_sc = (lon_sc - ((decimal_year - 2000.0) * 0.01397)) % 360.0
