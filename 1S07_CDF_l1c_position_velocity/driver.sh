...
if [ ! -d "$input" ]; then
  echo "1S07_CDF_l1c_position_velocity Error: Directory '$directory' not found."
  exit 1
fi
...
find "$input" -maxdepth 1 -type f -name "*.cdf" -mtime "$delaytime" -print0 | while IFS= read -r -d '' file; do
  ...
  if [ -f "$file" ]; then
      echo "1S07_CDF_l1c_position_velocity Processing file: $file"
  fi

  ofile="${file/#..\/input/./output}"
  ofile="${ofile/.cdf/}"
  echo "1S07_CDF_l1c_position_velocity Output H file: $ofile"
