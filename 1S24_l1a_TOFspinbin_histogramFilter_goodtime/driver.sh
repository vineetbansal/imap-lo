...
if [ ! -d "$input" ]; then
  echo "Error 1S24: Directory '$directory' not found."
  exit 1
fi
...
find "$input" -maxdepth 1 -type f -name "*.cdf" -mtime "$delaytime" -print0 | while IFS= read -r -d '' file; do
  ...
  if [ -f "$file" ]; then
      echo "1S24 Processing file: $file"
  fi
  ...
  base=$(basename "$file")
    ...
    if [ -z "$hk_file" ]; then
        echo "1S24 Warning: no matching HK file found for $file"
        echo "              expected pattern: $input_hk/imap_lo_l1b_nhk_${stem}_v*.cdf"
        continue
    fi

    echo "1S24 Using HK file: $hk_file"
  ...
  echo "1S24 Output files: $ofileH $ofileCO"

  ./runIMAP_fmv1_read_l1a_de.sh "$file" "$hk_file" "$ofileH" "$ofileCO" "$goodtime_file"
