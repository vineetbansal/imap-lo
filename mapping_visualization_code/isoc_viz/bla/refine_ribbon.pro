...
;  ..refine_ribbon
;
pro refine_ribbon,glon,glat,ribmap,linlon,linlat,ang,newlon,newlat,newang
...
for ic=1,nln-1 do begin
   ...
   if ii[0] ge 0 then begin
       ni = n_elements(ii)
       sumv = [0.0,0.0,0.0]
       for iii=0,nii-1 do begin
           if (ribmap[ii[iii]] gt 0.0) then begin
               sumv[*] += cvecg[*,ii[iii]]*ribmap[ii[iii]]
       end
       center = sumv/sqrt(total(sumv^2)+1.0e-15) 
