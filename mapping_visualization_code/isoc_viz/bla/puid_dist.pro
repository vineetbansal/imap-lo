...
; pro puid_dist.pro
;
; This procedure generates a pui distribution beyond the termination
...
pro puid_dist,                  $
	...
	uswu=uswu,              $ ; OPT_IN: upwind solar wind spd [cm/s] 
        ...
        comp=comp,              $ ; TS compression ratio
...
if not keyword_set(spda) then begin
    n = 1000
    spda = 0.0 - 2.0*uswu*dindgen(n)/(n-1.0)
end
...
for i=0, n-1 do begin
    ...
    if (w gt 1.0) then $
      dist[i] = 0.0 $
        ...
        if (w gt 0.0001) then $
          dist[i] = (beta*dens*1.5e13/(8.0*!dpi*uswu^4)) * $
          ( 1.0 / (rats * w^1.5 ) ) *exp(-lamb/(rats*w^1.5)) $
        else $
          dist[i] = 0.0
    end
end
