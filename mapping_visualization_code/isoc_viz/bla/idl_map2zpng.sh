...
USAGE="Usage: $ME fluxmap.txt expmap.txt output_tag fill 
                  p0lon p0lat min max title 

which generates a nice plot, output_tag.png,
that has the  fluxmap. This is like idl_map2png
but the quality of the output graphic is publication
ready. It also creates text file output_tag.fxnn.txt
...
cat > fuckme.txt <<EOF
    ...
    $opt1
    ips.zexpand=1 
    ips.mapname='Mollweide'
    ips.mapframe='ECLIPJ2000'
    ips.fs=40
    ips.mapgridllt=5
    ips.mapgridlls=0
