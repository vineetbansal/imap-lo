...
#    $python $pydir/calcEfficiencies-RATE-TOF-BD.py -f $fileRAWCNT -b $fileTOFBD -m $mcpLo $mcpHi > "$outdir"/mcpGainReport.txt
    ...
    echo "   4S4  hist_auto_ram.py input:" $file
    echo "   4S4  hist_auto_ram.py input de:" $file_de
    echo "   4S4  hist_auto_ram.py input hk:" $file_hk
    echo "   4S4  hist_auto_ram.py output:" $output
