...
if __name__ == "__main__":
    ...
    # ------------- Assemble hist rates ------------- #
    hist_latest = {}
    for file in Path('../input_l1b_histrates').glob("*.cdf"):
    ...
    # ------------- Assemble hk data ------------- #
    hk_latest = {}
    for hk_file in Path('../input_hk').glob("*.cdf"):
    ...
    # ------------- Goodtime Dataframe -------------- #
    df = pd.read_csv(
        '../input_goodtime/imap_lo_goodtimes.csv',
    ...
    esa_flags = df[
        ['esa1', 'esa2', 'esa3', 'esa4', 'esa5', 'esa6', 'esa7']].to_numpy().copy()
    ...
    for key in sorted(hist_latest):
        ...
        for element in ['Oxygen', 'Hydrogen']:
            ...
            for esa_name in ['ESA1', 'ESA2', 'ESA3', 'ESA4', 'ESA5', 'ESA6', 'ESA7']:
                ...
                mu_mom, dmu_mom, sigma_mom, dsigma_mom, peak_mom, dpeak_mom = compute_moments_wunc(
                    theta_local, rates_local, exposure_local)

                mu_fit, sigma_fit, peak_fit, bg_fit, mu_unc, sigma_unc, peak_unc, C_unc, A, A_unc = do_gaussian_fit_local_wunc(
                    theta_deg, rates, drates, half_width=30.0)
